...
window.onload = function() {
    ...
    for (i = 1; i <= rows * columns; i++) {
        pieces.push(i.toString()); //put "1" to "9" into the array
    }

    posDict = {};
        count = 1;
        for (i = 0; i < hexOutputArray.length; i++){
            posDict[i] = pieces[((parseInt(hexOutputArray[i], 16)) % 9)];
            console.log("i : " + i +  " , element: " + hexOutputArray[i] + " ,  dec : " + (parseInt(hexOutputArray[i], 16)) + " ,  dec mod 9 : " + ((parseInt(hexOutputArray[i], 16)) % 9));
        }
    

    // Salsa20-based shuffling
    var piecesInd = parseInt(hexOutputArray[i], 16%9);
    for (i = 0; i < pieces.length; i++) {
        if(pieces[piecesInd] != false){
